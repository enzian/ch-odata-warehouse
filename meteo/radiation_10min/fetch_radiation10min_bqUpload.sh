...
bq load --quote=\" --source_format=CSV --field_delimiter=";" --ignore_unknown_values --max_bad_records=10 ch_meteo.radiation_10min_staging trimmed.csv

bq query \
    --use_legacy_sql=false \
    '
MERGE `ch_meteo.radiation_10min` T
USING `ch_meteo.radiation_10min_staging` S
...
bq query \
    --use_legacy_sql=false \
    'DELETE `ch_meteo.radiation_10min_staging` WHERE true'
