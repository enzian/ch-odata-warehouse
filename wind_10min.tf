...
resource "google_cloud_run_v2_job" "meteo_wind_10min" {
  name     = "meteo-wind-10min-scraper"
    ...
    template {
      containers {
        ...
        command = [
          "bash",
          "-c",
          file("${path.root}/meteo/wind_10min/fetch_bqUpload.sh")
        ]
      }
...
resource "google_cloud_scheduler_job" "meteo_wind_10m" {
  name      = "meteo-wind-10m-scraping"
  schedule  = "*/10 * * * *"
  time_zone = "Europe/Zurich"

  http_target {
    http_method = "POST"
    uri         = "https://${google_cloud_run_v2_job.meteo_wind_10min.location}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${google_project.ch-odata-wh-project.project_id}/jobs/${google_cloud_run_v2_job.meteo_wind_10min.name}:run"
  ...
  depends_on = [
    ...
    google_cloud_run_v2_job.meteo_wind_10min,
    google_project_iam_binding.scrape_trigger_cloud_run_invoker
  ]
}

resource "google_bigquery_data_transfer_config" "meteo_ch_wind_10min_dedup" {
  project = google_project.ch-odata-wh-project.project_id
